...
1.  **Backend (Python):**
    ...
    -   If data persistence is needed, define a model in `app/models.py`, generate a sequential migration with `uv run stone db migrate -m "<action_slug>"`, and apply it with `uv run stone db upgrade`. Run transactions with `get_session()` from `app/database.py`.
